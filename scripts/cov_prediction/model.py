...
def update_plot_with_pred_cov(cov_x, cov_y, mux, muy, ax, do_pause=True, isok=1):
    ...
    for ped in range(0, len(mux)):
        for timestemp in range(1, len(mux[ped])):
            ax.plot([mux[ped][timestemp], mux[ped][timestemp-1]], [muy[ped][timestemp],
                                                                   muy[ped][timestemp-1]], colors[ped]+"-", markersize=1, label="input")
...
def calc_linear_covariance(x):
    ...
    for ped in peds:
        ...
        for step in range(peds[ped]["end_step"]+1-peds[ped]["start_step"]):
            peds[ped]["linear"][step] = peds[ped]["pose"][0] + \
                peds[ped]["vel"]*step
            ...
            for ped in ped_cov.keys():
                for timestemp in range(ped_cov[ped]["start_step"]+1, ped_cov[ped]["end_step"]):
                    ...
                    input_ = torch.from_numpy(
                        np.stack((cov_prev, pose_prev, pose_cur))).reshape(6, 1).float()
                    y_pred = model(input_.T)
                    loss = loss_fn(y_pred, torch.from_numpy(
                        ped_cov[ped]["cov"][timestemp]).float())
        ...
        if 0:
            for _ in range(10):
                ...
                for timestemp in range(1, 19):
                    ...
                    cov_list_x.append(cov_[0])
                    cov_list_y.append(cov_[1])
                update_plot_with_pred_cov(
                    [cov_list_x], [cov_list_y], [mux], [muy], ax)
