...
def optimisation(EPOCHS, lr, starting_poses, goals, robot_init_pose, param):
    for epoch_numb in range(0,EPOCHS):
            ...
            _, cost, stacked_trajectories_for_visualizer, _,_,_ = sequential((inner_data, cost, stacked_trajectories_for_visualizer, goals, param, robot_init_pose))
    ...
    with torch.no_grad():
        ...
        new_state_propagation=stacked_trajectories_for_visualizer[inner_data.shape[0]:inner_data.shape[0]*2].detach().clone()
...
if __name__ == '__main__':
    ...
    vis_goals_ped = Visualizer2('/goals/ped',               size=[0.1, 0.1, 0.5])
    vis_goals_rob = Visualizer2('/goals/rob',    color=1,   size=[0.1, 0.1, 0.5])
    vis_learning = Visualizer2("/peds/learning", color=2,   size=[0.2,0.2,1.0], with_text = False)
    # visualisation staff
    ...
    while not rospy.is_shutdown():
        ...
        starting_poses = optimisation(EPOCHS=5, lr=10**-4, starting_poses=starting_poses, goals=goals, robot_init_pose=robot_init_pose, param = param)
