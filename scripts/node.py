...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        rf, af = calc_forces(input_state, goal, param.pedestrians_speed, param.k)
        F = rf + af
        input_state = pose_propagation(
            F, input_state, param.DT, param.pedestrians_speed)
        cost += calc_cost_function(param.a, param.b, param.e, param.robot_speed,
                                   robot_pose, robot_goal, robot_init_pose, input_state[:, 0:2])
